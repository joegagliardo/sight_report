...
DEFAULT_LOGO_INSTRUCTION = """I want to generate a graphic similar to the attachment called template
LAYOUT RULES:  WHITE HEADER (top of page):  Left side: Google Cloud logo (cloud icon only, no text) + "TRIP Report" in black, font-size 20px, same line Below that (with extra line spacing): 
Company logo at 1/3 the height and width of the header row above it, left-aligned DARK BLUE BOX (fills remaining page width, background #0D2B63 or similar dark navy):  and found by using the attachment called company_logo 

Top-left heading: "Completed PLLJ Training Overview" in white, bold Below heading: Company name ("Rackspace") in bright blue (#4DA6FF or similar), bold, larger font 

TIMELINE: Horizontal line across the box with one entry per course, sorted earliest → latest.  - Each entry has:   1. A blue pill/bubble ABOVE the timeline line with the formatted date (e.g. "Jan 1, 2026")   2. A vertical connector line to the timeline.   3. A rounded blue card BELOW the timeline containing:     - Course title (top)     - Instructor photo found by matching the name of the instructor to the images fetched from the process_gcs_manifest_tool

 — show ONCE per card, inside the card only     - Attendee count as a number only (no label), with a group-people icon next to it  - Do NOT show instructor name text, do NOT show "Attendees:" label, do NOT show instructor photo above the date or outside the card -The horizontal line begins with a vertical connector line and ends with a vertical connector line. There must be as many vertical lines as there are dates and bubbles. And the first and last should be at the beginning and end of the line.  BOTTOM-LEFT of blue box: ROI Training logo (globe icon + "ROI Training" in white) Do NOT include the company logo inside the blue box OUTPUT: A graphic similar to the attached
"""
